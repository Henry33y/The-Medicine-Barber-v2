...
export async function signInWithGoogle() {
	...
	if (error) {
		...
		console.log('[supabaseClient] Google OAuth launched. Data present:', !!data);
		try {
			const url = data?.url || '';
			console.log('[supabaseClient] Provider URL:', url);
			const qIndex = url.indexOf('?');
			if (qIndex !== -1) {
				const qs = url.slice(qIndex + 1);
				const params = new URLSearchParams(qs);
				const providerRedirect = params.get('redirect_uri');
				if (providerRedirect) {
					console.log('[supabaseClient] Provider redirect_uri:', decodeURIComponent(providerRedirect));
					if (decodeURIComponent(providerRedirect).includes('localhost:3000')) {
						console.warn('[supabaseClient] Detected redirect_uri to localhost:3000. Supabase likely fell back to Site URL.\n- Add the Expo proxy URL to Auth → Redirect URLs:', redirectTo, '\n- Update Site URL to a non-localhost domain if not developing for web.');
					}
				}
			}
		} catch (_e) {
